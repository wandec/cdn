...
void execute3(vector<vector<edge> >& G, GSize& size, vector<int>& table, vector<int>& Gtable, vector<int>& min_flow, vector<int>& min_index, int min_sum, int serverCost, int& minfee){
	...
	vector<int> serverId(size.NC*5/4);
	vector<int> backupId(size.NC*5/4);
	...
	for (int k = 0; k < iter_outer; ++k) {
		...
		bool satisfied = true;
		int len=iter/2;
		double scale= (SHIFTP-START_SHIFT)/ (double)len;
		double disc_scale = (DISCARD-START_DISCARD) / (double)len;
		double shift_p = START_SHIFT, discard_p = START_DISCARD;
		vector<vector<int> > history;
		memset(used, 0, G.size()*sizeof(char));
		for (int i = 0; i < size.NC; ++i) {
		...
		for(int i = size.NC;i<serverId.size();++i){
			int r=rand()%size.NN;
			if(!used[r]){
				serverId[i]=Gtable[r];
				used[r]=1;
		...
		while (iter)  {
			...
			if(satisfied)	history.push_back(id);
				...
				for (int i = 0; i < mmin(serverId.size(), history[row].size()); ++i) {
					serverId[i] = history[row][i];
				}
				satisfied = true;
			}
